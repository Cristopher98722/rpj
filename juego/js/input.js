const Input = {
  ...
  virtual: {},
  touch: { device: false, move: null, aim: null, reviveTouches: [] },
  inited: false,
  ...
  reviveButtonRect() {
    return { cx: Config.CANVAS.w / 2, cy: Config.CANVAS.h - 250, r: 50 };
  },
  ...
  onTouchStart(canvas, e) {
    ...
    for (const t of e.changedTouches) {
      ...
      if (Game.state === 'playing') {
        ...
        const db = this.desaButtonRect();
        if (Math.hypot(p.x - db.cx, p.y - db.cy) <= db.r) {
          this.virtual['k'] = true;
          continue;
        }
        const rb = this.reviveButtonRect();
        if (Math.hypot(p.x - rb.cx, p.y - rb.cy) <= rb.r) {
          this.virtual['v'] = true;
          if (!this.touch.reviveTouches.includes(t.identifier)) this.touch.reviveTouches.push(t.identifier);
          continue;
        }
  ...
  onTouchMove(canvas, e) {
    ...
    for (const t of e.changedTouches) {
      ...
      if (this.touch.reviveTouches.includes(t.identifier)) {
        const rb = this.reviveButtonRect();
        if (Math.hypot(p.x - rb.cx, p.y - rb.cy) > rb.r) {
          this.touch.reviveTouches = this.touch.reviveTouches.filter((id) => id !== t.identifier);
        }
      }
    }
  },
  ...
  isReviveHeld() {
    return this.touch.reviveTouches.length > 0;
  },
