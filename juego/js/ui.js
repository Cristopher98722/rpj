const UI = {
  ...
  drawHUD(ctx, player, game) {
    ...
    if (game.multi) {
      ...
      let yy = 130;
      for (const ph of game.peers) {
        ctx.fillStyle = ph && ph.hero ? ph.hero.color : '#4fc3f7';
        ctx.fillText((ph.name || 'Aliado') + (ph.dead ? '  ·  CAÍDO' : ''), W - 24, yy);
        if (ph && !ph.dead) {
          const w = 150;
          const h = 7;
          const x = W - 24 - w;
      ...
      ctx.fillText(Net.connected ? (game.waveTotal ? 'Oleada: ' + (game.waveKills + game.peerWaveKills) + ' / ' + game.waveTotal + ' bajas' : Net.status) : Net.status, W - 24, yy);
      ctx.restore();
    }
    ...
    ctx.fillText(
      Input.touch.device
        ? 'Sticks: IZQ mover · DER atacar · Q/E/R habilidades · RISA · DESA · REVIVE'
        : 'WASD mover · Clic atacar · Q/E/R habilidades · F PODER · L risa · K desahuevate · V revivir · ESC menú',
      W - 20, H - 12);
  ...
  drawTouch(ctx, player) {
    ...
    const db = Input.desaButtonRect();
    ctx.save();
    ctx.globalAlpha = 0.7;
    ctx.fillStyle = '#1b2838';
    ctx.beginPath();
    ctx.arc(db.cx, db.cy, db.r, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = 'rgba(255,255,255,0.35)';
    ctx.lineWidth = 2;
    ctx.stroke();
    ...
    ctx.font = 'bold 12px "Segoe UI", Arial';
    ctx.fillStyle = '#fff';
    ctx.fillText('DESA', db.cx, db.cy + 5);
    ctx.restore();

    const needsRevive = Game.multi && Game.peers.some((p) => p.dead);
    if (needsRevive) {
      const rb = Input.reviveButtonRect();
      const pulse = 0.5 + 0.5 * Math.sin((Game.timeAlive || 0) * 10);
      ctx.save();
      ctx.globalAlpha = 0.75 + pulse * 0.25;
      ctx.fillStyle = '#1c3b25';
      ctx.beginPath();
      ctx.arc(rb.cx, rb.cy, rb.r, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.fillText('REVIVE', rb.cx, rb.cy + 5);
      ctx.restore();
    }
  ...
  drawLobby(ctx, net) {
    ...
    ctx.fillStyle = '#fff';
    ctx.fillText('Jugadores: ' + net.players + ' / ' + Config.MAX_PLAYERS, W / 2, H / 2 - 20);

    if (net.roster && net.roster.length) {
      ctx.textAlign = 'left';
      ctx.font = 'bold 14px "Segoe UI", Arial';
      ctx.fillStyle = 'rgba(255,255,255,0.75)';
      ctx.fillText('JUGADORES EN SALA:', 120, H / 2 - 40);
      ctx.font = '15px "Segoe UI", Arial';
      let yy = H / 2 - 14;
      for (const r of net.roster) {
        const isMe = r.idx === net.idx;
        const hero = HEROES.find((h) => h.id === r.hero);
        ctx.fillStyle = isMe ? '#ffd54f' : (hero ? hero.color : '#fff');
        ctx.fillText((isMe ? '▶ ' : '· ') + r.name + (isMe ? '  (TÚ)' : ''), 140, yy);
        yy += 24;
      }
      ctx.textAlign = 'center';
    }
  ...
  drawShop(ctx, player, game) {
    ...
    const yimg = Assets.cache['assets/img/yerald.jpeg'];
    if (yimg) {
      const fade = Utils.clamp((performance.now() - (game.shopOpenAt || 0)) / 700, 0, 1);
      const pulse = 0.5 + 0.5 * Math.sin((Game.timeAlive || 0) * 3);
      const cx = W - 100;
      const cy = 285 + Math.sin((Game.timeAlive || 0) * 2.5) * 4;
      const cr = 96;
      ctx.save();
      ctx.globalAlpha = fade;
      ctx.shadowColor = 'rgba(255,213,79,' + (0.45 + pulse * 0.3) + ')';
      ctx.shadowBlur = 30;
      ctx.fillStyle = '#1b2838';
      ctx.beginPath();
      ...
      ctx.clip();
      const scale = Math.max((cr * 2) / yimg.width, (cr * 2) / yimg.height);
      ctx.drawImage(yimg, cx - (yimg.width * scale) / 2, cy - (yimg.height * scale) / 2, yimg.width * scale, yimg.height * scale);
      ctx.restore();
      ctx.save();
      ctx.globalAlpha = fade;
      ctx.textAlign = 'center';
      ctx.font = 'bold 20px "Segoe UI", Arial';
      ctx.fillStyle = '#ffd54f';
      ctx.fillText('YERALD', cx, cy + cr + 32);
      ctx.restore();

      Shop.tickDialogue(performance.now());
      if (Shop.dialogue) {
        const dElapsed = performance.now() - Shop.dialogueStart;
        let a = 1;
        if (dElapsed < 350) a = dElapsed / 350;
        if (dElapsed > 5200) a = Math.max(0, 1 - (dElapsed - 5200) / 800);
        ctx.save();
        ctx.globalAlpha = fade * a;
        ctx.font = 'bold 15px "Segoe UI", Arial';
        const tw = ctx.measureText(Shop.dialogue).width;
        const bw = Math.min(360, tw + 44);
        const bh = 52;
        const bx = cx - bw / 2;
        const byb = cy - 146;
        ctx.fillStyle = '#fff8e1';
        Utils.roundRect(ctx, bx, byb, bw, bh, 14);
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(cx - 12, byb + bh - 2);
        ctx.lineTo(cx + 12, byb + bh - 2);
        ctx.lineTo(cx, byb + bh + 24);
        ctx.closePath();
        ctx.fill();
        ctx.fillStyle = '#4a3c12';
        ctx.textAlign = 'center';
        ctx.fillText(Shop.dialogue, cx, byb + bh / 2 + 5);
        ctx.restore();
      }
    }
  ...
  drawPixelNumber(ctx, x, y, str, px, color) {
    ctx.save();
    ...
    for (let c = 0; c < chars.length; c++) {
      const glyph = this.PIXEL_DIGITS[chars[c]];
      if (!glyph) continue;
      for (let row = 0; row < 7; row++) {
        const line = glyph[row];
        for (let col = 0; col < charW; col++) {
          if (line[col] === '1') {
            ctx.fillRect(x + c * (charW + 1) * px + col * px, y + row * px, px, px);
          }
        }
      }
    }
  ...
  drawCountdown(ctx, t, maxT, wave) {
    ...
    Utils.roundRect(ctx, bx - 8, by - 8, bw + 16, bh + 16, 14);
    ctx.fill();
    ...
    ctx.fillRect(bx + bw, by - 8, 8, 8);
    ctx.fillRect(bx - 8, by + bh, 8, 8);
    ctx.fillRect(bx + bw, by + bh, 8, 8);

    ctx.textAlign = 'center';
    ctx.font = 'bold 13px "Courier New", monospace';
    ctx.fillStyle = '#81c784';
    ctx.fillText('PROXIMA OLEADA ' + wave, W / 2, by + 24);

    const len = String(secs).length;
    const px = 6;
    const totalW = (len * 6 - 1) * px;
    const pulse = 1 + frac * 0.06;
    ctx.save();
    ctx.translate(W / 2, by + 52);
    ctx.scale(pulse, pulse);
    this.drawPixelNumber(ctx, -totalW / 2, -21, String(secs), px, '#ffd54f');
    ctx.restore();
    ...
    const barY = by + 76;
    const fill = Utils.clamp(maxT > 0 ? t / maxT : 0, 0, 1);
    const blocks = 14;
    ctx.fillStyle = 'rgba(0,0,0,0.55)';
    ...
    for (let i = 0; i < blocks; i++) {
      if (i / blocks <= fill) {
        ...
        ctx.fillRect(barX + i * (barW / blocks), barY, barW / blocks - 2, barH);
      }
    }
    ctx.restore();
  }
