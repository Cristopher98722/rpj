const UI = {
  ...
  drawHUD(ctx, player, game) {
    ...
    ctx.fillText(
      Input.touch.device
        ? 'Sticks fijos: IZQ mover · DER atacar · Q/E/R habilidades'
        : 'WASD mover · Clic atacar · Q/E/R habilidades · F PODER · ESC menú',
      W - 20, H - 12);
  ...
  drawShop(ctx, player, game) {
    ...
    layout.items.forEach((r, i) => {
      ...
      ctx.font = 'bold 20px "Segoe UI", Arial';
      ctx.fillStyle = '#fff';
      ctx.fillText(item.name, r.x + r.w / 2, r.y + 148);
    ...
    ctx.font = '14px "Segoe UI", Arial';
    ctx.fillStyle = 'rgba(255,255,255,0.7)';
    ctx.fillText('← → para elegir  ·  ENTER compra o continúa  ·  ESC continúa', W / 2, H - 16);
    ctx.restore();
  },
