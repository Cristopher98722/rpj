const UI = {
  ...
  drawShop(ctx, player, game) {
    ...
    if (yimg) {
      ...
      ctx.font = '13px "Segoe UI", Arial';
      ctx.fillStyle = 'rgba(255,255,255,0.7)';
      ctx.fillText('Te observa mientras compras...', cx, cy + cr + 52);
      ctx.restore();
    }
  ...
  drawCountdown(ctx, t, maxT, wave) {
    ...
    ctx.strokeStyle = '#3b5a44';
    ctx.lineWidth = 4;
    ctx.strokeRect(bx, by, bw, bh);
    ctx.strokeStyle = '#1c3b25';
    ctx.lineWidth = 2;
    ...
    ctx.fillText('PROXIMA OLEADA ' + wave, W / 2, by + 40);
    ctx.font = 'bold 13px "Courier New", monospace';
    ctx.fillStyle = 'rgba(255,255,255,0.7)';
    ctx.fillText('PREPARATE...', W / 2, by + 64);

    const len = String(secs).length;
    const totalW = (len * 6 - 1) * 14;
    const pulse = 1 + frac * 0.08;
    ctx.save();
    ctx.translate(W / 2, by + 155);
    ctx.scale(pulse, pulse);
    this.drawPixelNumber(ctx, -totalW / 2, -49, String(secs), 14, '#ffd54f');
    ctx.restore();
    ...
    ctx.fillStyle = 'rgba(255,255,255,0.6)';
    ctx.fillText('SEGUNDOS', W / 2, barY + barH + 22);
    ctx.restore();
  }
};
