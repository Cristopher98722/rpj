...
function leave(conn) {
  ...
  if (!room) return;
  const idx = room.players.indexOf(conn) + 1;
  room.players = room.players.filter((p) => p !== conn);
  conn.room = null;
  const roster = room.players.map((p, i) => ({ idx: i + 1, name: p.name, hero: p.hero }));
  for (const p of room.players) {
    wsSend(p, { t: 'roster', list: roster });
    wsSend(p, { t: 'peerLeave', idx });
  }
...
function handleMessage(conn, msg) {
  ...
  if (msg.t === 'join') {
    ...
    if (room.players.length >= MAX_PLAYERS) {
      wsSend(conn, { t: 'error', msg: 'Sala llena (máximo ' + MAX_PLAYERS + ' jugadores)' });
      return;
    }
    conn.room = code;
    ...
    const roster = room.players.map((p, i) => ({ idx: i + 1, name: p.name, hero: p.hero }));
    wsSend(conn, { t: 'join', ok: true, room: code, idx, count: idx, roster });
    for (const p of room.players) {
      if (p !== conn) {
        wsSend(p, { t: 'roster', list: roster });
        wsSend(p, { t: 'peerJoin', idx, name: conn.name, hero: conn.hero });
      }
    }
    return;
  }
  ...
  if (msg.t === 'state' || msg.t === 'kill' || msg.t === 'cleared' || msg.t === 'next' ||
      ...
      msg.t === 'desa' || msg.t === 'miku' || msg.t === 'death' || msg.t === 'revive' ||
      msg.t === 'esync' || msg.t === 'dmg' || msg.t === 'phit') {
    const room = roomOf(conn);
    if (!room) return;
    for (const p of room.players) {
